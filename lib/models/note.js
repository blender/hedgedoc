...
const { constrainObject } = require('constrain-object')
...
module.exports = function (sequelize, DataTypes) {
  ...
  Note.generateWebTitle = function (title) {
    title = !title || title === 'Untitled' ? 'Blender Notes - Collaborative notes' : title + ' - Blender Notes'
    return title
  }
  ...
  function splitFrontmatter (str) {
    ...
    return {
      yaml: str.slice(0, metaEnd.index),
      markdown: str.slice(matcher.lastIndex)
    }
  }
  ...
  Note.extractMeta = function (content) {
    ...
    try {
      ...
      if (parts) {
        const parsed = yaml.load(parts.yaml)
        const constrained = constrainObject(parsed, {
          onCycle: 'omit',
          maxDepth: 4
        })
        obj = {
